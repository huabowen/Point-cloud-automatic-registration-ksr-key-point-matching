...
pcl::PFHSignature125 com_pfh(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normals, pcl::search::KdTree<pcl::PointXYZ>::Ptr tree, int keypoint, float radius) {
	pcl::PointCloud<pcl::PFHSignature125>::Ptr pfhs(new pcl::PointCloud<pcl::PFHSignature125>());
	pcl::PFHEstimation<pcl::PointXYZ, pcl::Normal, pcl::PFHSignature125> pfh;
	pcl::PointCloud<pcl::PointXYZ>::Ptr key(new pcl::PointCloud<pcl::PointXYZ>);
	key->push_back(cloud->points[keypoint]);
	...
	pfh.compute(*pfhs);
	pcl::PFHSignature125 p(pfhs->points[0]);
	return p;
}
...
float com_feature_dist(pcl::PFHSignature125 f1, pcl::PFHSignature125 f2) {
	float dist = 0.0f;
	for (int i = 0; i < 125; i++) {
...
pcl::CorrespondencesPtr com_correspondence(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_source, pcl::PointCloud<pcl::PointXYZ>::Ptr key_source, pcl::PointCloud<pcl::PFHSignature125>::Ptr feature_source,
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_target, pcl::PointCloud<pcl::PointXYZ>::Ptr key_target, pcl::PointCloud<pcl::PFHSignature125>::Ptr feature_target, float dis, float leaf_size) {

	pcl::CorrespondencesPtr corrs(new pcl::Correspondences());

	pcl::KdTreeFLANN<pcl::PFHSignature125> feature_target_kdtree;
	...
	for (size_t i = 0; i < key_source->size(); i++)
	{
		...
		feature_target_kdtree.nearestKSearch(feature_source->points[i], 5, corr_indices, corr_dists);
		...
		for (int j = 0; j < corr_indices.size(); j++) {
			if (sqrt(corr_dists[j]) < dis)
			{
				...
				if (dist1 < 5.0f* leaf_size&&dist2 < 5.0f* leaf_size&&feature_dist1 < dis&&feature_dist2 < dis&&dist_angle < 10.0f) {
					//show_key_corr(cloud_source, key_source,
					//	key_source->points[key_source_indices[0]], key_source->points[key_source_indices[1]], key_source->points[key_source_indices[2]],
					//	cloud_target, key_target,
					//	key_target->points[key_target_indices[0]], key_target->points[key_target_indices[1]], key_target->points[key_target_indices[2]]);
...
int main(int argc, char** argv){
	...
	start = GetTickCount();
	float leaf_size = 0;
	*cloud_source = *point_cloud_to_num(cloud_source, 20000);
	end = GetTickCount();
	...
	*normals_source = *normal_estimation_OMP(cloud_source, leaf_size*5.0f);
	...
	if (flag) {
		...
		*key_source = *key_detect_u(cloud_source, 10.0*leaf_size);//对比实验
		*features_source = *com_pfh_feature(cloud_source, normals_source, key_source, 10.0f *leaf_size);//对比实验
	}
	...
	*normals_target = *normal_estimation_OMP(cloud_target, leaf_size*5.0f);
	...
	if (flag) {
		...
		*key_target = *key_detect_u(cloud_target, 10.0*leaf_size);//对比实验
		*features_target = *com_pfh_feature(cloud_target, normals_target, key_target, 10.0f *leaf_size);//对比实验
	}
	...
	//pcl::PointCloud<pcl::PFHSignature125>::Ptr new3_features_source(new pcl::PointCloud<pcl::PFHSignature125>);
	//pcl::PointCloud<pcl::PFHSignature125>::Ptr new3_features_target(new pcl::PointCloud<pcl::PFHSignature125>);
	...
	for (int i = 0; i < indice.size(); i++) {
		new2_key_source->push_back(new_key_source->points[indice[i]]);
		new2_key_target->push_back(new_key_target->points[indice[i]]);
	}
	pcl::PointCloud<pcl::PointXYZ>::Ptr new2_cloud_source(new pcl::PointCloud<pcl::PointXYZ>);
	*new2_cloud_source = *cloud_source;
	for (int i = 0; i < new2_cloud_source->size(); i++) {
		new2_cloud_source->points[i].x += 300.0f* leaf_size;
		new2_cloud_source->points[i].y += 300.0f* leaf_size;
	}
	for (int i = 0; i < new2_key_source->size(); i++) {
		new2_key_source->points[i].x += 300.0f* leaf_size;
		new2_key_source->points[i].y += 300.0f* leaf_size;
	}
	////////////////////显示对应点连线//////////////////////////////////////////////////////////////////////
	pcl::visualization::PCLVisualizer line("line");
	line.setBackgroundColor(255, 255, 255);
	line.addPointCloud(new2_cloud_source, pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>(new2_cloud_source, 0.0, 0, 0), "new4_cloud_source");
	...
	line.addPointCloud<pcl::PointXYZ>(new2_key_source, pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>(new2_key_source, 0, 0, 0), "new4_key_source");
	line.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, "new4_key_source");

	line.addPointCloud<pcl::PointXYZ>(new2_key_target, pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>(new2_key_target, 0, 0, 0), "new4_key_target");
	line.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, "new4_key_target");

	for (int i = 0; i < new2_key_source->size(); i++)
	{
		line.addLine(new2_key_source->points[i], new2_key_target->points[i], 0, 0, 0, to_string(i));
